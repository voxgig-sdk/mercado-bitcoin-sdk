...
from mercadobitcoin_sdk.core.helpers import get_ctx_prop, to_map
...
class MercadoBitcoinContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = MercadoBitcoinControl()
        ...
        if isinstance(sp, MercadoBitcoinSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, MercadoBitcoinResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, MercadoBitcoinResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return MercadoBitcoinOperation({})
    ...
    def make_error(self, code, msg):
        return MercadoBitcoinError(code, msg, self)
