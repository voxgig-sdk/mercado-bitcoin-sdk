...
class TestBalanceEntity:
    ...
    def test_should_stream(self):
        ...
        base = MercadoBitcoinSDK.test(seed, None)
        seen = list(base.Balance(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = MercadoBitcoinSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Balance(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MERCADO_BITCOIN_TEST_BALANCE_ENTID JSON to run live")
        client = setup["client"]
...
def _balance_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MERCADO_BITCOIN_TEST_BALANCE_ENTID"))
