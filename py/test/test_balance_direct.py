...
class TestBalanceDirect:
    ...
    def test_should_direct_list_balance(self):
        setup = _balance_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-balance", "live" if setup["live"] else "unit")
        if _skip:
