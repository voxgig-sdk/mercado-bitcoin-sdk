# MercadoBitcoin SDK test runner
...
class MercadoBitcoinTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MercadoBitcoinTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MercadoBitcoinTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MercadoBitcoinTestRunner.getenv("MERCADO_BITCOIN_TEST_LIVE")
        override = MercadoBitcoinTestRunner.getenv("MERCADO_BITCOIN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MercadoBitcoinTestRunner.getenv(key)
        ...
        explain = MercadoBitcoinTestRunner.getenv("MERCADO_BITCOIN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MERCADO_BITCOIN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MercadoBitcoinTestRunner._test_control is not None:
            return MercadoBitcoinTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MercadoBitcoinTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MercadoBitcoinTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MercadoBitcoinTestRunner.load_test_control()
...
def load_env_local():
    MercadoBitcoinTestRunner.load_env_local()


def env_override(m):
    return MercadoBitcoinTestRunner.env_override(m)


def entity_data(v):
    return MercadoBitcoinTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MercadoBitcoinTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MercadoBitcoinTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MercadoBitcoinTestRunner.load_test_control()


def live_client_options():
    return MercadoBitcoinTestRunner.live_client_options()


def live_delay_ms():
    return MercadoBitcoinTestRunner.live_delay_ms()
