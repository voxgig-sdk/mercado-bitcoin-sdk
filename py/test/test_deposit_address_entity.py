    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MERCADOBITCOIN_TEST_DEPOSIT_ADDRESS_ENTID JSON to run live")
        client = setup["client"]
        ...
        deposit_address_ref01_data_dt0_loaded, err = deposit_address_ref01_ent.load(deposit_address_ref01_match_dt0, None)
        assert err is None
...
def _deposit_address_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MERCADOBITCOIN_TEST_DEPOSIT_ADDRESS_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MERCADOBITCOIN_TEST_DEPOSIT_ADDRESS_ENTID"))
