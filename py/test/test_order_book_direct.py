...
class TestOrderBookDirect:

    def test_should_direct_load_order_book(self):
        setup = _order_book_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-order_book", "live" if setup["live"] else "unit")
        if _skip:
...
def _order_book_direct_setup(mockres):
