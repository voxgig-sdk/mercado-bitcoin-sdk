...
class TestTickerDirect:
    ...
    def test_should_direct_list_ticker(self):
        setup = _ticker_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ticker", "live" if setup["live"] else "unit")
        if _skip:
