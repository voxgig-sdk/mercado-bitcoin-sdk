...
class TestCandleDirect:

    def test_should_direct_load_candle(self):
        setup = _candle_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-candle", "live" if setup["live"] else "unit")
        if _skip:
...
def _candle_direct_setup(mockres):
