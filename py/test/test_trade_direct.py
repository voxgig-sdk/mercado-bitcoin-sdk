...
class TestTradeDirect:

    def test_should_direct_load_trade(self):
        setup = _trade_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-trade", "live" if setup["live"] else "unit")
        if _skip:
...
def _trade_direct_setup(mockres):
