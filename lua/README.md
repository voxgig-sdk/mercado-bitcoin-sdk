...
The Lua SDK for the MercadoBitcoin API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Balance(nil):list(nil, nil)
...
local result, err = client:MercadoBitcoin(nil):load(
  { id = "test01" }, nil
)
