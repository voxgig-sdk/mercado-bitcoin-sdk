...
describe("OrderEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Order(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Order(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Order(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MERCADOBITCOIN_TEST_ORDER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local order_ref01_ent = client:Order(nil)
    local order_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.order"), "order_ref01"))

    local order_ref01_data_result, err = order_ref01_ent:create(order_ref01_data, nil)
    assert.is_nil(err)
    ...
    assert.is_nil(err)
    local order_ref01_data_dt0_load_result = helpers.to_map(order_ref01_data_dt0_loaded)
    ...
    -- REMOVE
    local order_ref01_match_rm0 = {
      id = order_ref01_data["id"],
    }
    local _, err = order_ref01_ent:remove(order_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local order_ref01_match_rt0 = {}

    local order_ref01_list_rt0_result, err = order_ref01_ent:list(order_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(order_ref01_list_rt0_result)
...
function order_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/order/OrderTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read order test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("MERCADOBITCOIN_TEST_ORDER_ENTID")
