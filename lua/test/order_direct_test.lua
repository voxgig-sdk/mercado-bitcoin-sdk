...
describe("OrderDirect", function()
  it("should direct-list-order", function()
    local setup = order_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-order", setup.live and "live" or "unit")
  ...
  it("should direct-load-order", function()
    local setup = order_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-order", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function order_direct_setup(mockres)
