...
local TradeEntity = {}
TradeEntity.__index = TradeEntity


function TradeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TradeEntity)
  self._name = "trade"
  self._client = client
...
function TradeEntity:get_name()
  return self._name
end


function TradeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TradeEntity.new(self._client, opts)
end
...
function TradeEntity:mark_deleted()
  self._deleted = true
end


function TradeEntity:deleted()
  return true == self._deleted
end


function TradeEntity:data_set(args)
  if args ~= nil then
...
function TradeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TradeEntity:match_set(args)
  if args ~= nil then
...
function TradeEntity:match_get()
...
function TradeEntity:stream(action, args, callopts)
...
function TradeEntity:_run_op(ctx, post_done)
  local utility = self._utility
