...
local OrderEntity = {}
OrderEntity.__index = OrderEntity


function OrderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrderEntity)
  self._name = "order"
  self._client = client
...
function OrderEntity:get_name()
  return self._name
end


function OrderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrderEntity.new(self._client, opts)
end
...
function OrderEntity:mark_deleted()
  self._deleted = true
end


function OrderEntity:deleted()
  return true == self._deleted
end


function OrderEntity:data_set(args)
  if args ~= nil then
...
function OrderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrderEntity:match_set(args)
  if args ~= nil then
...
function OrderEntity:match_get()
...
function OrderEntity:stream(action, args, callopts)
...
function OrderEntity:_run_op(ctx, post_done)
  local utility = self._utility
