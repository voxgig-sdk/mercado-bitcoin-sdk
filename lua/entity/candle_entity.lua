-- MercadoBitcoin SDK Candle entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CandleEntity = {}
CandleEntity.__index = CandleEntity


function CandleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CandleEntity)
  self._name = "candle"
  self._client = client
...
function CandleEntity:get_name()
  return self._name
end


function CandleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CandleEntity.new(self._client, opts)
end
...
function CandleEntity:mark_deleted()
  self._deleted = true
end


function CandleEntity:deleted()
  return true == self._deleted
end


function CandleEntity:data_set(args)
  if args ~= nil then
...
function CandleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CandleEntity:match_set(args)
  if args ~= nil then
...
function CandleEntity:match_get()
...
function CandleEntity:stream(action, args, callopts)
...
---@param reqmatch CandleLoadMatch
---@param ctrl? table
---@return Candle
---@return string? err
function CandleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CandleEntity:_run_op(ctx, post_done)
  local utility = self._utility
