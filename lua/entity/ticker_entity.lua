...
local TickerEntity = {}
TickerEntity.__index = TickerEntity


function TickerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TickerEntity)
  self._name = "ticker"
  self._client = client
...
function TickerEntity:get_name()
  return self._name
end


function TickerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TickerEntity.new(self._client, opts)
end
...
function TickerEntity:mark_deleted()
  self._deleted = true
end


function TickerEntity:deleted()
  return true == self._deleted
end


function TickerEntity:data_set(args)
  if args ~= nil then
...
function TickerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TickerEntity:match_set(args)
  if args ~= nil then
...
function TickerEntity:match_get()
...
function TickerEntity:stream(action, args, callopts)
...
function TickerEntity:_run_op(ctx, post_done)
  local utility = self._utility
