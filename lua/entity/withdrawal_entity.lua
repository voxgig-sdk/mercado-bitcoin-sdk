-- MercadoBitcoin SDK Withdrawal entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WithdrawalEntity = {}
WithdrawalEntity.__index = WithdrawalEntity


function WithdrawalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WithdrawalEntity)
  self._name = "withdrawal"
  self._client = client
...
function WithdrawalEntity:get_name()
  return self._name
end


function WithdrawalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WithdrawalEntity.new(self._client, opts)
end
...
function WithdrawalEntity:mark_deleted()
  self._deleted = true
end


function WithdrawalEntity:deleted()
  return true == self._deleted
end


function WithdrawalEntity:data_set(args)
  if args ~= nil then
...
function WithdrawalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WithdrawalEntity:match_set(args)
  if args ~= nil then
...
function WithdrawalEntity:match_get()
...
function WithdrawalEntity:stream(action, args, callopts)
...
---@param reqdata WithdrawalCreateData
---@param ctrl? table
---@return Withdrawal
---@return string? err
function WithdrawalEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function WithdrawalEntity:_run_op(ctx, post_done)
  local utility = self._utility
