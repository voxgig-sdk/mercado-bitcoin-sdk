...
-- by passing it.
function MercadoBitcoinSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MercadoBitcoinSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MercadoBitcoinSDK: graphql: " .. msg
    res.graphql = errors
  end
