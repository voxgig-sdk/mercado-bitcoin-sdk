...
func TestTradeDirect(t *testing.T) {
	t.Run("direct-load-trade", func(t *testing.T) {
		setup := tradeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-trade", _mode); _shouldSkip {
			if _reason == "" {
...
func tradeDirectSetup(mockres any) *tradeDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"MERCADO_BITCOIN_TEST_TRADE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MERCADO_BITCOIN_TEST_TRADE_ENTID"]; ok {
