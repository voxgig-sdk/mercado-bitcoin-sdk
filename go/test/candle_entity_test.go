...
func TestCandleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		candleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.candle")))
...
func candleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MERCADO_BITCOIN_TEST_CANDLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MERCADO_BITCOIN_TEST_CANDLE_ENTID"])
