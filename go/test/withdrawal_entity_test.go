...
func TestWithdrawalEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		withdrawalRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "withdrawal"}), "withdrawal_ref01"))

		withdrawalRef01DataResult, err := withdrawalRef01Ent.Create(withdrawalRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		withdrawalRef01Data = core.ToMapAny(entityData(withdrawalRef01DataResult))
...
func withdrawalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MERCADO_BITCOIN_TEST_WITHDRAWAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MERCADO_BITCOIN_TEST_WITHDRAWAL_ENTID"])
