...
func TestOrderDirect(t *testing.T) {
	t.Run("direct-list-order", func(t *testing.T) {
		setup := orderDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-order", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-order", func(t *testing.T) {
		setup := orderDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-order", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type orderDirectSetupResult struct {
...
func orderDirectSetup(mockres any) *orderDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"MERCADO_BITCOIN_TEST_ORDER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MERCADO_BITCOIN_TEST_ORDER_ENTID"]; ok {
		...
		return &orderDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &orderDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
