...
func TestOrderBookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		orderBookRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.order_book")))
...
func order_bookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MERCADO_BITCOIN_TEST_ORDER_BOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MERCADO_BITCOIN_TEST_ORDER_BOOK_ENTID"])
