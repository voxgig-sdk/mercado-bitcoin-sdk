...
func TestTickerDirect(t *testing.T) {
	t.Run("direct-list-ticker", func(t *testing.T) {
		setup := tickerDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-ticker", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-ticker", func(t *testing.T) {
		setup := tickerDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-ticker", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type tickerDirectSetupResult struct {
	client *sdk.MercadoBitcoinSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func tickerDirectSetup(mockres any) *tickerDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"MERCADO_BITCOIN_TEST_TICKER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MERCADO_BITCOIN_TEST_TICKER_ENTID"]; ok {
		...
		return &tickerDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &tickerDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
