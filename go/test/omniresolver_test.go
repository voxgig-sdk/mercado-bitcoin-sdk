...
import (
	...
	omni "github.com/voxgig-sdk/mercado-bitcoin-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MercadoBitcoinError); is {
				out := map[string]any{
					"name":    "MercadoBitcoinError",
...
func omniCtx(arg any, client *sdk.MercadoBitcoinSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MercadoBitcoinSDK); is {
			client = live
