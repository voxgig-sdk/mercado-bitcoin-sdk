...
func (sdk *MercadoBitcoinSDK) OptionsMap() map[string]any {
...
func (sdk *MercadoBitcoinSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MercadoBitcoinSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MercadoBitcoinSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MercadoBitcoinSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MercadoBitcoinSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MercadoBitcoinSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MercadoBitcoinSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MercadoBitcoinSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MercadoBitcoinSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MercadoBitcoinSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *MercadoBitcoinSDK {
	...
	sdk := NewMercadoBitcoinSDK(sdkopts)
	sdk.Mode = "test"
