// Command MercadoBitcoin-cli is a CLI + REPL for the MercadoBitcoin SDK,
...
const prompt = "mercado-bitcoin"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "balance candle deposit_address order order_book ticker trade withdrawal"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MERCADO_BITCOIN_APIKEY (api key), MERCADO_BITCOIN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MERCADO_BITCOIN_APIKEY carries the API key and
	// MERCADO_BITCOIN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MERCADO_BITCOIN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MERCADO_BITCOIN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMercadoBitcoinSDK(opts)
