...
AQL-driven command-line client **and** interactive REPL for the MercadoBitcoin
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mercado-bitcoin-cli)
make build

# 2. See usage (words, entities, env vars)
./mercado-bitcoin-cli --help
...
./mercado-bitcoin-cli list balance
./mercado-bitcoin-cli list candle

# 5. Override the API base URL for a single call
MERCADO_BITCOIN_BASE=https://api.example.com ./mercado-bitcoin-cli list balance

# 6. No arguments -> interactive REPL
./mercado-bitcoin-cli
mercado-bitcoin> list balance
mercado-bitcoin> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mercado-bitcoin-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mercado-bitcoin-cli list balance
   ```
...
Run with no arguments to open a REPL (prompt `mercado-bitcoin>`). Each line is
evaluated as its own AQL expression:

```text
$ ./mercado-bitcoin-cli
mercado-bitcoin> list balance
mercado-bitcoin> /help
mercado-bitcoin> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `balance`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mercado-bitcoin-cli`. |
...
(`./mercado-bitcoin-cli <expr>`) and interactively (the REPL), and expressions compose the
