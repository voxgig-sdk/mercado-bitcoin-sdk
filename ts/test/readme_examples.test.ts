...
//     (\`new MercadoBitcoinSDK(...)\` / \`MercadoBitcoinSDK.test(...)\` become
//     \`MercadoBitcoinSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MercadoBitcoinSDK'
...
const TEST_SEED = {"entity":{"balance":{"test01":{"id":"test01"}},"candle":{"test01":{"id":"test01"}},"deposit_address":{"test01":{"id":"test01"}},"order":{"test01":{"id":"test01"}},"order_book":{"test01":{"id":"test01"}},"ticker":{"test01":{"id":"test01"}},"trade":{"test01":{"id":"test01"}},"withdrawal":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MercadoBitcoinSDK(...)\` and any
// \`MercadoBitcoinSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MercadoBitcoinSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MercadoBitcoinSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
