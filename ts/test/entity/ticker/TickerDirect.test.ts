...
describe('TickerDirect', async () => {
  ...
  test('direct-load-ticker', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ticker', setup.live)) return
  ...
  test('direct-list-ticker', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ticker', setup.live)) return
    const { client, calls } = setup
