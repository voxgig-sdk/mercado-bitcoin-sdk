...
describe('OrderBookDirect', async () => {
  ...
  test('direct-load-order_book', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-order_book', setup.live)) return
    if (skipIfMissingIds(t, setup, ["symbol01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MERCADO_BITCOIN_TEST_ORDER_BOOK_ENTID']
