...
describe('OrderDirect', async () => {
  ...
  test('direct-load-order', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-order', setup.live)) return
  ...
  test('direct-list-order', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-order', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MERCADO_BITCOIN_TEST_ORDER_ENTID']
