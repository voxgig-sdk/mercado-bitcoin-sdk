...
// MercadoBitcoin SDK feature test
//
...
class FtCtrl extends MercadoBitcoinControl
{
...
class FtClient
{
    ...
    private MercadoBitcoinUtility $utility;

    public function __construct(MercadoBitcoinUtility $utility, array $options)
    {
    ...
    public function get_utility(): MercadoBitcoinUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MercadoBitcoinContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MercadoBitcoinContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MercadoBitcoinContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MercadoBitcoinConfig::make_feature($name);
    ...
    private static function buildUrl(MercadoBitcoinSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MercadoBitcoinError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MercadoBitcoinResponse($fetched);
        }
    ...
    private function fail(MercadoBitcoinContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MercadoBitcoinContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
