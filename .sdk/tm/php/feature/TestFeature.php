...
// MercadoBitcoin SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MercadoBitcoinTestFeature extends MercadoBitcoinBaseFeature
{
    ...
    public function init(MercadoBitcoinContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MercadoBitcoinContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MercadoBitcoinParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MercadoBitcoinContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MercadoBitcoinContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MercadoBitcoinParam::call($ctx, $k);
