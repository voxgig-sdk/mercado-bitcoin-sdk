...
module MercadoBitcoinUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MercadoBitcoinResponse.new({})
    ...
    if fetch_err
      ...
      response = MercadoBitcoinResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MercadoBitcoinResponse.new(fetched)
    else
