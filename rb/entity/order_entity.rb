...
class OrderEntity
  ...
  def make
    opts = @_entopts.dup
    OrderEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MercadoBitcoinHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Order, Hash] the current Order data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MercadoBitcoinHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Order fields)
  def match_get
  ...
  # Load a single Order.
  #
  # @param reqmatch [OrderLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Order.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Order, Hash] the loaded Order; raises MercadoBitcoinError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MercadoBitcoinHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [OrderListMatch, Hash, nil] match filter (any subset of
  #   Order fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Order>, Array] the matching Order items; raises MercadoBitcoinError on failure
  ...
  # Create a new Order.
  #
  # @param reqdata [OrderCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Order, Hash] the created Order; raises MercadoBitcoinError on failure
  ...
  #
  # @param reqmatch [OrderRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [Order, Hash] the removed Order; raises MercadoBitcoinError on failure
