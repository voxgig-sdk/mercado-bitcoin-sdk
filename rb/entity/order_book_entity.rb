...
class OrderBookEntity
  ...
  def make
    opts = @_entopts.dup
    OrderBookEntity.new(@_client, opts)
  end
  ...
  # @return [OrderBook, Hash] the current OrderBook data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of OrderBook fields)
  def match_get
  ...
  #
  # @param reqmatch [OrderBookLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.OrderBook.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [OrderBook, Hash] the loaded OrderBook; raises MercadoBitcoinError on failure
