...
class TradeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = trade_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "trade." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MERCADO_BITCOIN_TEST_TRADE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    trade_ref01_data = nil
    if trade_ref01_data_raw.length > 0
    ...
    trade_ref01_data_dt0_load_result = Helpers.to_map(trade_ref01_data_dt0_loaded.respond_to?(:data_get) ? trade_ref01_data_dt0_loaded.data_get : trade_ref01_data_dt0_loaded)
...
def trade_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "trade", "TradeTestData.json")
  ...
  entid_env_raw = ENV["MERCADO_BITCOIN_TEST_TRADE_ENTID"]
