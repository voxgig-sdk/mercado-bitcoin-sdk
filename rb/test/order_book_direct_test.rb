...
class OrderBookDirectTest < Minitest::Test
  def test_direct_load_order_book
    setup = order_book_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-order_book", setup[:live] ? "live" : "unit")
    if _should_skip
...
def order_book_direct_setup(mockres)
