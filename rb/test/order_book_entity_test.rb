...
class OrderBookEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = order_book_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "order_book." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MERCADO_BITCOIN_TEST_ORDER_BOOK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    order_book_ref01_data = nil
    if order_book_ref01_data_raw.length > 0
...
def order_book_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "order_book", "OrderBookTestData.json")
  ...
  entid_env_raw = ENV["MERCADO_BITCOIN_TEST_ORDER_BOOK_ENTID"]
