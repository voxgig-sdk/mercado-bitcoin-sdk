...
class DepositAddressEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MERCADO_BITCOIN_TEST_DEPOSIT_ADDRESS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def deposit_address_basic_setup(extra)
  ...
  entid_env_raw = ENV["MERCADO_BITCOIN_TEST_DEPOSIT_ADDRESS_ENTID"]
