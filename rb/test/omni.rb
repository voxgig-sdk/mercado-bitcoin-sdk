...
module MercadoBitcoinOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if MercadoBitcoinOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = MercadoBitcoinOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| MercadoBitcoinOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || MercadoBitcoinOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = MercadoBitcoinSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = MercadoBitcoinError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { MercadoBitcoinOmni.errify(err) }
