...
class WithdrawalEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MERCADO_BITCOIN_TEST_WITHDRAWAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    withdrawal_ref01_data = Helpers.to_map(withdrawal_ref01_data_result.respond_to?(:data_get) ? withdrawal_ref01_data_result.data_get : withdrawal_ref01_data_result)
...
def withdrawal_basic_setup(extra)
  ...
  entid_env_raw = ENV["MERCADO_BITCOIN_TEST_WITHDRAWAL_ENTID"]
