...
class CandleDirectTest < Minitest::Test
  def test_direct_load_candle
    setup = candle_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-candle", setup[:live] ? "live" : "unit")
    if _should_skip
...
def candle_direct_setup(mockres)
