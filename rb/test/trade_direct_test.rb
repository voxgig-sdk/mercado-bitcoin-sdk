...
class TradeDirectTest < Minitest::Test
  def test_direct_load_trade
    setup = trade_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-trade", setup[:live] ? "live" : "unit")
    if _should_skip
...
def trade_direct_setup(mockres)
