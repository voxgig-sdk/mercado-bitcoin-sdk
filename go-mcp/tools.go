...
type Args struct {
	Entity string         `json:"entity" jsonschema:"balance | candle | deposit_address | order | order_book | ticker | trade | withdrawal"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MercadoBitcoinSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "mercado-bitcoin_list",
		Description: "List records from MercadoBitcoin. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "mercado-bitcoin_load",
		Description: "Load a single record from MercadoBitcoin. " +
...
func runOp(client *sdk.MercadoBitcoinSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MercadoBitcoinSDK, name string) (sdk.MercadoBitcoinEntity, error) {
	switch strings.ToLower(name) {
	case "balance":
		return client.Balance(nil), nil
	case "candle":
		return client.Candle(nil), nil
	case "deposit_address":
		return client.DepositAddress(nil), nil
	case "order":
		return client.Order(nil), nil
	case "order_book":
		return client.OrderBook(nil), nil
	case "ticker":
		return client.Ticker(nil), nil
	case "trade":
		return client.Trade(nil), nil
	case "withdrawal":
		return client.Withdrawal(nil), nil
