...
[MCP](https://modelcontextprotocol.io) server exposing the MercadoBitcoin SDK as
two agent tools — `mercado-bitcoin_list` and `mercado-bitcoin_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mercado-bitcoin-mcp)
make build

# 2. Provide credentials via the environment
export MERCADO_BITCOIN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mercado-bitcoin \
  -- /absolute/path/to/mercado-bitcoin-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mercado-bitcoin-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mercado-bitcoin_list: first page of records
{ "entity": "balance" }
{ "entity": "balance", "query": { } }

// mercado-bitcoin_load: one record by id
{ "entity": "candle", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mercado-bitcoin-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mercado-bitcoin \
     -- "$PWD"/dist/*/mercado-bitcoin-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mercado-bitcoin_list` and `mercado-bitcoin_load` tools now appear
   in new sessions. Ask the agent to *"list balance using mercado-bitcoin"*
   and it calls `mercado-bitcoin_list` with `{"entity":"balance"}`.
...
```sh
export MERCADO_BITCOIN_APIKEY=sk_live_xxx            # API key
export MERCADO_BITCOIN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mercado-bitcoin_list` tool
...
### Call the `mercado-bitcoin_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mercado-bitcoin_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mercado-bitcoin_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MERCADO_BITCOIN_APIKEY` | API key sent with every request. |
| `MERCADO_BITCOIN_BASE` | Optional override of the API base URL. |

### Entities

The 8 entities valid as the `entity` argument:

balance | candle | deposit_address | order | order_book | ticker | trade | withdrawal

### Smoke test via HTTP (raw JSON-RPC)

```sh
./mercado-bitcoin-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mercado-bitcoin_load","arguments":{"entity":"candle","query":{"id":1}}}}'
```
