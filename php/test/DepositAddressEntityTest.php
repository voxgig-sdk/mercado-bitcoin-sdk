...
class DepositAddressEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MERCADOBITCOIN_TEST_DEPOSIT_ADDRESS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function deposit_address_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MERCADOBITCOIN_TEST_DEPOSIT_ADDRESS_ENTID");
