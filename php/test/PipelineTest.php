...
// MercadoBitcoin SDK pipeline test
//
...
{
    private static function utility(): MercadoBitcoinUtility
    {
        return new MercadoBitcoinUtility();
    }
    ...
    private static function ctx(array $over = []): MercadoBitcoinContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MercadoBitcoinContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MercadoBitcoinOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MercadoBitcoinError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MercadoBitcoinOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MercadoBitcoinMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MercadoBitcoinOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MercadoBitcoinMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MercadoBitcoinOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MercadoBitcoinMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MercadoBitcoinSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MercadoBitcoinMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MercadoBitcoinResponse([]),
            'result' => new MercadoBitcoinResult([]),
        ]);
        [, $err] = MercadoBitcoinMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec([]),
            'response' => null,
            'result' => new MercadoBitcoinResult([]),
        ]);
        [, $err] = MercadoBitcoinMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec(['step' => 's']),
            'response' => new MercadoBitcoinResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MercadoBitcoinResult([]),
        ]);
        [, $err] = MercadoBitcoinMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec(['step' => 's']),
            'response' => new MercadoBitcoinResponse(self::resp(200, ['v' => 1])),
            'result' => new MercadoBitcoinResult([]),
        ]);
        [, $err] = MercadoBitcoinMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec(['step' => 's']),
            'response' => new MercadoBitcoinResponse(self::resp(200, ['v' => 2])),
            'result' => new MercadoBitcoinResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MercadoBitcoinMakeResponse::call($ctx);
    ...
    {
        $preset = new MercadoBitcoinResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec([]),
            'response' => new MercadoBitcoinResponse([]),
            'result' => new MercadoBitcoinResult([]),
        ...
        [$got, $err] = MercadoBitcoinMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MercadoBitcoinResult([])]);
        [, $err] = MercadoBitcoinMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MercadoBitcoinSpec([]), 'result' => null]);
        [, $err] = MercadoBitcoinMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MercadoBitcoinSpec(['step' => 's']),
            'result' => new MercadoBitcoinResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MercadoBitcoinOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MercadoBitcoinMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MercadoBitcoinSpec(['step' => 's']),
            'result' => new MercadoBitcoinResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MercadoBitcoinOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MercadoBitcoinMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MercadoBitcoinResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MercadoBitcoinSpec([]),
            'result' => new MercadoBitcoinResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MercadoBitcoinMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MercadoBitcoinMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MercadoBitcoinError('boom', 'boom');
        $utility->fetcher = function (MercadoBitcoinContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MercadoBitcoinSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MercadoBitcoinMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MercadoBitcoinContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MercadoBitcoinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MercadoBitcoinResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        MercadoBitcoinMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MercadoBitcoinContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MercadoBitcoinSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MercadoBitcoinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MercadoBitcoinResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MercadoBitcoinSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MercadoBitcoinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MercadoBitcoinMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MercadoBitcoinMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MercadoBitcoinResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MercadoBitcoinDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MercadoBitcoinResult(['ok' => false])]);
        $this->expectException(MercadoBitcoinError::class);
        MercadoBitcoinDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MercadoBitcoinResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MercadoBitcoinMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MercadoBitcoinMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MercadoBitcoinBaseFeature();
        $b = new MercadoBitcoinBaseFeature();
        MercadoBitcoinFeatureAdd::call($ctx, $a);
        MercadoBitcoinFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MercadoBitcoinBaseFeature
    {
        $f = new MercadoBitcoinBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MercadoBitcoinFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MercadoBitcoinContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MercadoBitcoinSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MercadoBitcoinPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        MercadoBitcoinPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MercadoBitcoinResponse(['headers' => null]),
            'result' => new MercadoBitcoinResult([]),
        ]);
        MercadoBitcoinResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        MercadoBitcoinResultBody::call($ctx);
