...
//    only - a typed MercadoBitcoinContext there would make every ctx
...
final class MercadoBitcoinOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MercadoBitcoinContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MercadoBitcoinSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MercadoBitcoinResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MercadoBitcoinError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MercadoBitcoinResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MercadoBitcoinError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MercadoBitcoinError(is_string($code) ? $code : '', $msg);
    }
