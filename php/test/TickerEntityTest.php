...
class TickerEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MercadoBitcoinSDK::test(null, null);
        $ent = $testsdk->Ticker(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Ticker(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Ticker(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = ticker_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ticker." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MERCADO_BITCOIN_TEST_TICKER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ticker_ref01_data = null;
        if (count($ticker_ref01_data_raw) > 0) {
...
function ticker_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ticker/TickerTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ticker01", "ticker02", "ticker03"] as $k) {
    ...
    $entid_env_raw = getenv("MERCADO_BITCOIN_TEST_TICKER_ENTID");
