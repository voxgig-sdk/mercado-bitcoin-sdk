...
// MercadoBitcoin SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class MercadoBitcoinMakeError
{
    public static function call(?MercadoBitcoinContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MercadoBitcoinContext([], null);
        }
        $op = $ctx->op ?? new MercadoBitcoinOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MercadoBitcoinResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MercadoBitcoinError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MercadoBitcoinError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MercadoBitcoinError) {
