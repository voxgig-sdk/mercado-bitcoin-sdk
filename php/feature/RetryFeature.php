...
// MercadoBitcoin SDK retry feature
...
class MercadoBitcoinRetryFeature extends MercadoBitcoinBaseFeature
{
    ...
    public function init(MercadoBitcoinContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MercadoBitcoinContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MercadoBitcoinContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MercadoBitcoinContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
