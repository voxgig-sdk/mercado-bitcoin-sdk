...
class MercadoBitcoinSDK
{
        ...
        if ($feature_opts) {
            ...
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MercadoBitcoinHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MercadoBitcoinFeatures::has_feature($fname)) {
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MercadoBitcoinError($op . "_allow",
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MercadoBitcoinError("graphql_error",
                "MercadoBitcoinSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
